...
Basically, Terraform checks with the Cloud provider what's running at the moment, so it can get an up to date view of what the infrastructure actually looks like.
Then it figures out what it needs to do, reconciling what's actually running, with what we want to be running (the desired state).
...
One of the strength of Terraform is its ecosystem.
...
Below is a Terraform configuration file declaring the Azure provider:
...
It lets you generate Terraform configuration files (and maybe https://www.pulumi.com/[Pulumi] and https://github.com/Azure/bicep[Bicep] in the future) for Azure (and who knows, maybe other Cloud providers in the future).
...
This creates a Maven structure with all the needed Quarkus configuration as well as some generated code.
...
Let's package the application in an exectuable Uber-JAR.
...
Now that we have built the executable JAR, to run the application is just a matter of executing:
...
For that, we can either use the NubesGen web site or a cUrl command.

=== Generating with the Web Site

If you decide to generate the Terraform configuration files from the https://nubesgen.com/[NubesGen web site], you need to manually pick up what you need for your infrastructure.
In our case, you need to give a name to the project (eg. `quarkus-nubesgen-terraform`), pick up a region and choose _Quarkus_ from the combobox.
...
.Generating Terraform Files with NubesGen Web Site
image::2021-07-nubesgen.png[]

=== Generating with a cURL Command

If, like me, you prefer the command line, NubesGen allows you to download these configuration files throught a cUrl command.
If you click on _or copy cURL script_ you will get the following:
...
* `main.tf`:  Contains the main set of configuration for our module.
* `variables.tf`: Contains the variable definitions for our module.
* `outputs.tf`: Contains the output definitions for our module.
...
Basically, at the root of the `terraform` directory the files are used to define the needed providers (Azure in our case) and agregate the other Terraform files.
Under the sub directory `modules` you will find all the needed modules of our infrastructure (application, databases, etc.).
...
The, the `main.tf` file aggregates all the other modules.
...
But it also downloads the needed GO executables needed by the configuration (here the `terraform-provider-azurerm_v2.66.0_x5` executable):

.Generated Temporary Files
image::2021-07-terraform-init.png[]

You can also check if the configuration is valide with the `validate` command and show the state of the execution plan:
...
If you have https://graphviz.org[GraphViz] installed, you can even generate a visual representation of either a configuration or execution plan with the https://www.terraform.io/docs/cli/commands/graph.html[graph] command:
...
If at that moment, you are not logged in to Azure and want to define the plan, you will get the following error.
...
To avoid the error, make sure you are logged in to Azure with the following command:
...
As you can see, the method `apply` displays two very important information:
the URL of where we can target our application and the Azure resource groupe.
More on that below.
...
You can log into the Azure Portal at https://portal.azure.com and check the resources.
You should see the application as well as the plan:

.Azure Portal
image::2021-07-azure-console[]
...
A `terraform show` command will show... nothing.
...
Its language is simple, but because Terraforma has an extension mechanism, its ecosystem is huge.
