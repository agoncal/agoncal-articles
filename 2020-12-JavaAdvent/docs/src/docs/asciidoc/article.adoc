...
So we will develop Santa its own microservice (called _Santa_), with its own database, to schedule his coming deliveries.
...
For example, we could get the schedule for the previous year.
